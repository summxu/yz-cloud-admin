  ...
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="{row}">
          ...
          <el-button type="danger" size="mini" @click="reject(row)">撤销</el-button>
    ...
    <el-dialog title="团队成员列表" :visible.sync="dialogFormVisible">
      <el-table :data="numbers" style="width: 100%;" max-height="350" v-loading="numberLoading">
        ...
        <el-table-column label="签到时间" width="auto" align="left">
          <template slot-scope="scope">
            <span type="success">{{scope.row.sign_in}}</span>
          </template>
        </el-table-column>
        <el-table-column label="签退时间" width="auto" align="left">
          <template slot-scope="scope">
            <span type="success">{{scope.row.sign_out}}</span>
          </template>
        </el-table-column>
...
<script>
import { need_index, need_type, check_need, need_member, ce_need, need_top } from '@/api/yunzhijia'
...
export default {
  ...
  methods: {
    ...
    agree (row) {
      check_need({ id: row.id }).then(res => {
        let tempArr = this.list.filter(item => {
          return item.id == row.id
        })
        tempArr[0].status = '招募中'
      })
    },
    reject (row) {
